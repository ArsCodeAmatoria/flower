...
import { getSceneCountStats, getScriptPageStarts, getTotalScreenplayPages } from "@/data/script";
...
import { BeatNavSidebar } from "@/sections/script/BeatNavSidebar";
import { SceneDescriptionSidebar } from "@/sections/script/SceneDescriptionSidebar";
...
export function ScriptDraft2Cockpit({
  ...
  const sceneCount = useMemo(() => getSceneCountStats(pages, page), [pages, page]);
  ...
  const beatStructure = useMemo(() => flattenFlowerBeatsForNav(), []);
  const sceneTitle = useMemo(() => {
    const nav = beatStructure[page];
    if (nav) return `${nav.actLabel} · Beat ${String(nav.beat.num).padStart(2, "0")} — ${nav.beat.title}`;
    const sc = current?.elements.find((e) => e.type === "scene");
    return sc?.text ?? getPageTitle(current, labels);
  }, [current, labels, page, beatStructure]);
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ...
      { root, rootMargin: "-12% 0px -45% 0px", threshold: [0, 0.2, 0.4, 0.6] },
    );
    const attach = () => pageRefs.current.forEach((ref) => ref && observer.observe(ref));
    attach();
  ...
  return (
    <div className="flex min-h-0 w-full flex-1 overflow-hidden">
      ...
      <div className="relative flex min-w-0 flex-1 flex-col overflow-hidden">
        ...
        >
          ...
          <div className="mx-auto max-w-xl px-4 py-6 pb-16">
            ...
            {pages.map((p, idx) => {
              const structure = beatStructure[idx];
              return (
              <div
                key={p.id}
                id={`script-beat-${idx}`}
      ...
      <SceneDescriptionSidebar pages={pages} activePageIndex={page} />
    </div>
  );
}
