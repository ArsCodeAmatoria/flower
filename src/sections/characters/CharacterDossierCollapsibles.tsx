...
function Collapsible({
  ...
  return (
    <div
      className="mb-2 rounded-xl border border-zinc-200 bg-white"
      data-section={id}
    >
      <button
        type="button"
      ...
      >
        <span
          className="text-[10px] font-bold uppercase tracking-[0.18em] text-zinc-800"
      ...
      {open && <div className="border-t border-zinc-200 px-3 pb-3 pt-1">{children}</div>}
    </div>
  );
}
...
function AppliedBlock({ bullets }: { bullets: string[] }) {
  ...
  return (
    <div className="mt-2 rounded-lg border border-red-200 bg-red-50/80 px-2.5 py-2">
      <p className="mb-1.5 text-[8px] font-bold uppercase tracking-[0.2em] text-red-800">
        Applied
      </p>
      <ul className="list-inside list-disc space-y-1 text-[9px] leading-snug text-zinc-700">
        {bullets.map((line, j) => (
...
function LabeledList({ label, items }: { label: string; items: string[] }) {
  return (
    <div className="mb-3 last:mb-0">
      <p className="mb-1 text-[8px] font-bold uppercase tracking-[0.15em] text-zinc-500">{label}</p>
      <ul className="list-inside list-disc space-y-0.5 text-[9px] leading-snug text-zinc-700">
        {items.map((x, j) => (
  ...
  return (
    <div className="space-y-2 pt-2">
      <Collapsible id="character_save_the_cat_rules" title="Save the Cat — character rules" defaultOpen>
        <p className="mb-3 text-[9px] leading-relaxed text-zinc-600" style={{ fontFamily: "var(--font-screenplay)" }}>
          Read-only craft reminders. Each rule pairs with how it lands on this character.
        </p>
        <div className="space-y-4">
          {CHARACTER_SAVE_THE_CAT_RULES.map((rule, i) => (
            <div key={rule.id} className="rounded-lg border border-zinc-200 bg-zinc-50 px-2.5 py-2.5">
              <p className="text-[10px] font-semibold text-zinc-900" style={{ fontFamily: "var(--font-cinematic)" }}>
                {i + 1}. {rule.title}
              </p>
              <p className="mt-1 text-[9px] leading-snug text-zinc-600">{rule.principle}</p>
              {rule.craftReminders.length > 0 && (
                <p className="mt-1.5 text-[8px] uppercase tracking-wider text-zinc-500">
      ...
      <Collapsible id="character_mckee_rules" title="McKee — character rules" defaultOpen>
        <p className="mb-3 text-[9px] text-zinc-600" style={{ fontFamily: "var(--font-screenplay)" }}>
      ...
      <Collapsible id="character_arc_tracker" title="Arc tracker">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-stretch">
          {(
            ...
            <div
              key={label}
              className="flex-1 rounded-lg border border-zinc-200 bg-zinc-50 px-3 py-2.5"
            >
              <p className="mb-1 text-[8px] font-bold uppercase tracking-[0.2em] text-red-800">
                {label}
              </p>
              <p className="text-[10px] leading-snug text-zinc-800" style={{ fontFamily: "var(--font-screenplay)" }}>
                {text}
              </p>
            </div>
          ))}
